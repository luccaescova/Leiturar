...
import { checkInternet } from './controllers/CheckInternet';
...
mongoose.connect(MONGO_URI)
  ...
  status.then(result => {
    console.log(`Status da Internet: ${result.online ? "Online" : "Offline"}`);})
    .catch(err => {
        console.log("Erro ao verificar status da internet: "+ err);
    });